...
function setup() {
  ...
  trex.scale = 0.5;
  trex.debug = true
  trex.setCollider("circle",-7,-7,40)
  
  ground = createSprite(200,height - 20,9000 ,20);
  ground.addImage("ground",groundImage);
  ground.x = 0;
  ground.velocityX = -(6 + 3*score/100);
  
  gameOver = createSprite(width/3,height - 120);
  gameOver.addImage(gameOverImg);
  
  restart = createSprite(width/3,height - 60);
...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if((keyDown("space")|| touches.length > 0) && trex.y >= height - 49) {
      trex.velocityY = -12;
      touches = []
    }
